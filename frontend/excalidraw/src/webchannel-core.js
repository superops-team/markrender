/**
 * MarkRender WebChannel 核心通信库 (Excalidraw版本)
...
window.handlePythonMessage = function(action, data, requestId) {
    if (window.WebChannelManager) {
        window.WebChannelManager.handlePythonMessage(action, data, requestId);
    } else {
        console.warn('WebChannelManager未初始化，延迟处理消息:', action, data, requestId);
        // 延迟处理，等待WebChannelManager初始化完成
        setTimeout(() => {
            if (window.WebChannelManager) {
                window.WebChannelManager.handlePythonMessage(action, data, requestId);
            } else {
                console.error('WebChannelManager仍未初始化，无法处理消息:', action, data, requestId);
            }
        }, 100);
    }
};
...
window.WebChannelManager = (function() {
    ...
    // 处理Python消息
    function handlePythonMessage(action, data, requestId) {
        try {
    ...
    function initWebChannel(pageType = 'unknown') {
        try {
            ...
            if (document.readyState === 'loading') {
                ...
                logger.info('页面仍在加载中，等待DOMContentLoaded事件');
                ...
                // 页面已解析完成但仍在加载子资源，等待load事件
                logger.info('页面已解析完成但仍在加载子资源，等待load事件');
                return new Promise((resolve, reject) => {
        ...
        try {
            ...
            try {
                return state.backendInterface.dispatch_request(JSON.stringify(request))
                    .then(responseJson => {
                        try {
                            return handlePythonResponse(responseJson);
                        } catch (error) {
            ...
            logger.error('sendToPython执行异常:', error);
            // 不抛出异常，确保不影响主流程
    ...
    return {
        ...
        handlePythonMessage: handlePythonMessage,
        notifyChannelReady: function() {
            state.isChannelReady = true;
            logger.info('WebChannel手动标记为就绪');
...
document.addEventListener('DOMContentLoaded', function() {
    try {
        ...
        console.error('DOMContentLoaded处理失败:', error);
    }
});
