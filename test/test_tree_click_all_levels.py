...
class TestWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("QuickPick Tree All Levels Click Test")
        ...
        # 添加说明标签
        info_label = QLabel("测试说明：\n1. 点击任意层级的文件夹item，应该能展开/折叠\n2. 点击任意层级的文件item应该能切换到对应文件\n3. 所有层级（包括第三层、第四层等）都应该能正常工作")
        ...
        # 创建测试数据按钮
        test_data_btn = QPushButton("创建多层测试数据（5层结构）")
    ...
    def create_test_data(self):
        ...
        try:
            ...
            self.log_message(f"创建第三层文件夹，ID: {third_folder_id}")
            
            # 在第三层文件夹下创建第四层文件夹
            fourth_folder_id = self.manager.create_folder("测试第四层文件夹", parent_id=third_folder_id)
            self.log_message(f"创建第四层文件夹，ID: {fourth_folder_id}")
            
            # 在第四层文件夹下创建第五层文件夹
            fifth_folder_id = self.manager.create_folder("测试第五层文件夹", parent_id=fourth_folder_id)
            self.log_message(f"创建第五层文件夹，ID: {fifth_folder_id}")
            
            # 在第五层文件夹下创建文件
            file_id = self.manager.create_file(
                title="测试第五层文件.md",
                content="# 第五层文件\n\n这是第五层文件夹中的文件",
                parent_id=fifth_folder_id,
                page_type='markdown'
            )
            self.log_message(f"创建第五层文件，ID: {file_id}")
            
            # 在第四层文件夹下创建文件
            file2_id = self.manager.create_file(
                title="测试第四层文件.md",
                content="# 第四层文件\n\n这是第四层文件夹中的文件",
                parent_id=fourth_folder_id,
                page_type='markdown'
            )
            self.log_message(f"创建第四层文件，ID: {file2_id}")
