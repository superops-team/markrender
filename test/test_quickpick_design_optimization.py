...
class TestWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("QuickPick Design Optimization Test")
        ...
        # 添加说明标签
        info_label = QLabel("测试说明：\n1. 验证优化后的QuickPick色彩搭配是否符合TDesign设计原则\n2. 验证树形结构的视觉层次是否清晰\n3. 验证布局和间距是否合理")
    ...
    def create_test_data(self):
        ...
        try:
            ...
            # 在子文件夹下创建不同类型的文件
            md_file_id = self.manager.create_file(
                title="测试Markdown文件.md",
                content="# 测试Markdown文件\n\n这是测试Markdown文件的内容",
                parent_id=sub_folder_id,
                page_type='markdown'
            )
            self.log_message(f"创建Markdown文件，ID: {md_file_id}")
            
            pdf_file_id = self.manager.create_file(
                title="测试PDF文件.pdf",
                content="PDF内容",
                parent_id=sub_folder_id,
                page_type='pdf'
            )
            self.log_message(f"创建PDF文件，ID: {pdf_file_id}")
