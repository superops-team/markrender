...
def main():
    ...
    page_id = 'test_page'
    comm_manager = BackendInterface(page_id)
    ...
    def test_communication():
        ...
        def check_function_exists(result):
            print(f'handleBackendMessage函数类型: {result}')
            
            if result == 'function':
                print('✅ handleBackendMessage函数已正确定义')
                # 测试发送消息到前端
                ...
                print(f'❌ handleBackendMessage函数未定义，类型: {result}')
            
        view.page().runJavaScript('typeof window.handleBackendMessage', check_function_exists)
