...
class BackendInterface(QObject):
    ...
    def dispatch_request(self, request_json):
        ...
        try:
            ...
            handler_tuple = self.handlers.get(request.action)
    ...
    def send_message(self, action: str, data: dict = None, callback=None):
        ...
        try:
            ...
            # 构造JavaScript代码，调用前端的handleBackendMessage函数
            # 使用json.dumps确保所有参数正确转义
            js_code = f"window.handleBackendMessage({json.dumps(action)}, {json.dumps(data)}, {json.dumps(request_id) if request_id else 'null'});"
    ...
    def _send_web_response(self, callback_id, result):
        ...
        js_code = f"window.handleBackendResponse({json.dumps(response)});"
