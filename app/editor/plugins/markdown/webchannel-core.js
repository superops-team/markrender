...
window.WebChannelManager = (function() {
    ...
    window.handleBackendMessage = function(action, data, requestId) {
        logger.debug('收到Backend消息:', action, data, 'requestId:', requestId);
    ...
    registerMessageHandler('setCurrentItemId', (data, requestId) => {
    ...
    // WebChannel初始化（带重试机制）
    function initWebChannel(pageType = 'markdown') {
        ...
        if (!window.qt || !window.qt.webChannelTransport) {
            ...
            if (state.retryCount < state.maxRetries) {
                ...
                logger.error('makrdown WebChannel初始化最终失败');
            }
