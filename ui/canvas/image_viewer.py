...
class ImageViewer(QtWidgets.QGraphicsView):
    ...
    def __init__(self, parent):
        ...
        self.brush_cursor = self.create_inpaint_cursor(
            'brush', self.brush_size)
        self.eraser_cursor = self.create_inpaint_cursor(
        ...
        if event.type() in (
            ...
            if len(touch_points) == 2:
                ...
                current_scale_factor = (
                    ...
                    QLineF(
                        touchPoint0.startPos(),
                        touchPoint1.startPos()).length())
                if any(
                        tp.state() == QEventPoint.State.Released for tp in touch_points):
    ...
    def mousePressEvent(self, event):
        ...
        if event.button() == QtCore.Qt.LeftButton:
            ...
            if blk_item or rect_item:
                ...
                if self._in_rotate_ring(sel_item, scene_pos):
                    ...
                    sel_item.rot_handle = self.get_rotate_handle(
                        outer_rect, local_pos, angle)
        ...
        if self.current_tool in ['brush', 'eraser'] and self.hasPhoto():
            if self.photo.contains(scene_pos):
                ...
                if self.current_tool == 'eraser':
                    self.before_erase = [
                        pcb.save_path_properties(item) for item in self._scene.items() if isinstance(
        ...
        if self.current_tool == 'pan' or isinstance(
                clicked_item, TextBlockItem) or scroll:
            super().mousePressEvent(event)
    ...
    def mouseMoveEvent(self, event):
        ...
        if blk_item or rect_item:
            ...
            if handle:
                # get_cursor_for_position already applies rotation‐adjustment
            ...
            if sel_item.rotating and sel_item.center_scene_pos:
                ...
                if self._in_rotate_ring(sel_item, scene_pos):
                    ...
                    cursor = self.get_rotation_cursor(
                        outer_rect, local_pos, angle)
        ...
        if self.current_tool == 'box':
            ...
            if self.box_mode:
                ...
                # Handle negative dimensions when dragging from right to left
                # or bottom to top
    ...
    def mouseReleaseEvent(self, event):
        ...
        if event.button() == QtCore.Qt.LeftButton:
            ...
            # reset the view’s cursor so it doesn’t stay stuck on a resize
            # shape
            self.viewport().setCursor(QtCore.Qt.CursorShape.ArrowCursor)
        ...
        if self.current_tool in ['brush', 'eraser']:
            ...
            if self.current_tool == 'eraser':
                self.after_erase = [
                    pcb.save_path_properties(item) for item in self._scene.items() if isinstance(
                        item, QGraphicsPathItem)]
                command = EraseUndoCommand(
                    self, self.before_erase, self.after_erase)
                self.command_emitted.emit(command)
                self.after_erase = []
                self.before_erase = []
        ...
        if self.current_tool == 'box':
            if self.box_mode:
                ...
                ).width() > 0 and self.current_rect.rect().height() > 0:
    ...
    def get_rotation_cursor(self, outer_rect, pos, angle):
        inner_rect = outer_rect.adjusted(self.rotate_margin_max,
                                         ...
                                         -self.rotate_margin_max,
                                         -self.rotate_margin_max)
        handle = self._resolve_rotate_handle(
            inner_rect, outer_rect, pos, angle)
    ...
    def erase_at(self, pos: QtCore.QPointF):
        ...
        for item in items:
            if isinstance(
                    item,
                    QtWidgets.QGraphicsPathItem) and item != self.photo:
                path = item.path()
                ...
                if brush_color == "#80ff0000":  # generated stroke
                    ...
                    for i in range(36):
                        ...
                        point = QtCore.QPointF(
                            pos.x() + self.eraser_size * math.cos(angle),
                    ...
                    for item in items:
                        if isinstance(
                                item, QtWidgets.QGraphicsPathItem) and item != self.photo:
                            path = item.path()
                            intersected_path = path.intersected(
                                precise_erase_path)
                            if not intersected_path.isEmpty():
                                new_path = QtGui.QPainterPath(path)
                                new_path = new_path.subtracted(
                                    intersected_path)
                    ...
                    while i < element_count:
                        ...
                        if not erase_path.contains(point):
                            if element_type == QtGui.QPainterPath.ElementType.MoveToElement:
                                ...
                                # Handle curves by adding the next two control
                                # points
                                if i + 2 < element_count:
                                    ...
                                    if not (
                                            erase_path.contains(c1_point) or erase_path.contains(c2_point)):
                                        new_path.cubicTo(
                                            point, c1_point, c2_point)
                                i += 2  # Skip the control points
                        else:
                            # If the point is within the erase area, start a
                            # new subpath if the next point is outside
                            if (i + 1) < element_count:
                                next_element = path.elementAt(i + 1)
                                next_point = QtCore.QPointF(
                                    next_element.x, next_element.y)
    ...
    def get_cv2_image(self, paint_all=False, include_patches=True):
        ...
        if paint_all:
            ...
            for item in self._scene.items():
                ...
                if (isinstance(item, QtWidgets.QGraphicsPixmapItem) and
                    ...
                    painter.drawPixmap(
                        int(pos.x()), int(pos.y()), item.pixmap())
        ...
        if byte_count != expected_size:
            print(
                f"QImage sizeInBytes: {byte_count}, Expected size: {expected_size}")
            print(
                f"Image dimensions: ({width}, {height}), Format: {
                    qimage.format()}")
            raise ValueError(
                f"Byte count mismatch: got {byte_count} but expected {expected_size}")

        ptr = qimage.bits()

        # Convert memoryview to a numpy array considering the complete data
        # with padding
    ...
    def generate_mask_from_strokes(self):
        ...
        human_qimage = QtGui.QImage(
            width, height, QtGui.QImage.Format_Grayscale8)
        generated_qimage = QtGui.QImage(
            width, height, QtGui.QImage.Format_Grayscale8)
        ...
        for text_block in state.get('text_items_state', []):
            ...
            if 'transform_origin' in text_block and text_block['transform_origin']:
                text_item.setTransformOriginPoint(
                    QtCore.QPointF(*text_block['transform_origin']))
    ...
    def create_inpaint_cursor(self, cursor_type, size):
        ...
        if cursor_type == "brush":
            ...
            # Outline with some transparency
            painter.setPen(QColor(0, 0, 0, 127))
        else:
            # Default to some color if unknown type
