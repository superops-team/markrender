...
rot_svg_path = os.path.join(
    svg_root, 'dayu_widgets/static/rotate-arrow-top.svg')
...
class RotateHandleCursors:
    ...
    def load_cursor_image(self, path):
        ...
        if path.lower().endswith('.svg'):
            ...
            if renderer.isValid():
                image = QtGui.QImage(
                    self.size, self.size, QtGui.QImage.Format_ARGB32)
    ...
    def initialize_cursors(self):
        ...
        if base_image is not None:
            for handle, angle in rotations.items():
                ...
                rotated_image = base_image.transformed(
                ...
                self.cursors[handle] = QtGui.QCursor(
                    pixmap, self.size // 2, self.size // 2)
        else:
