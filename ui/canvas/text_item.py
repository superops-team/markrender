...
class TextBlockItem(QGraphicsTextItem):
    ...
    def update_text_format(self, attribute, value):
        ...
        format_operations = {
            'color': lambda cf,
            v: cf.setForeground(v),
            'font': lambda cf,
            v: cf.setFont(v),
            'size': lambda cf,
            v: cf.setFontPointSize(v),
            'bold': lambda cf,
            v: cf.setFontWeight(
                QFont.Bold if v else QFont.Normal),
            'italic': lambda cf,
            v: cf.setFontItalic(v),
            'underline': lambda cf,
            v: cf.setFontUnderline(v),
        }
    ...
    def mousePressEvent(self, event):
        if not self.editing_mode:
            ...
            if event.button() == Qt.LeftButton:
                ...
                self.resize_handle = self.get_handle_at_position(
                    local_pos, self.boundingRect())
    ...
    def resize_item(self, pos):
        ...
        rotated_delta_x = scene_delta.x() * math.cos(angle_rad) - \
            scene_delta.y() * math.sin(angle_rad)
        rotated_delta_y = scene_delta.x() * math.sin(angle_rad) + \
            scene_delta.y() * math.cos(angle_rad)
    ...
    def get_selected_text_properties(self, cursor: QTextCursor):
        ...
        # Get outline properties from the last (most recent) containing
        # selection
        outline_properties = None
        if containing_outlines:
            # Get the last one from the list
            latest_outline = containing_outlines[-1]
            outline_properties = {
                'outline': True,
        ...
        for pos in range(start, end):
            ...
            properties['text_color'].add(
                char_format.foreground().color().name())

        # Convert sets to single values if all elements are the same, otherwise
        # set to None
