...
def parse_db_orm(orm):
    ...
    return {
        "name": "ROOT" if hasattr(
            orm,
            "parent") and orm.parent is None else orm.name,
        "icon": utils.icon_formatter(orm),
        "get_children": lambda x: [
            parse_db_orm(orm) for orm in getattr(
                x,
                orm_map.get(
                    x.__tablename__,
                    None)) if orm.active],
        "has_children": lambda x: hasattr(
            x,
            orm_map.get(
                x.__tablename__,
                None)),
...
def parse_path(path):
    ...
    return {
        ...
        "icon": utils.icon_formatter(
            request_file("icon-browser.png")),
        "get_children": lambda x: [
            parse_path(
                os.path.join(
                    path,
                    i)) for i in os.listdir(path) if os.path.isdir(
                        os.path.join(
                            path,
                            i))],
        "has_children": lambda x: next(
            (True for i in os.listdir(path) if os.path.isdir(
                os.path.join(
                    path,
                    i))),
            False,
...
class MBaseButton(QWidget):
    ...
    def slot_show_menu(self):
        menu = QMenu(self)
        data_list = self.data_dict.get(
            "get_children")(self.data_dict.get("data"))
        for sub_obj in data_list:
            action = menu.addAction(sub_obj.get("icon"), sub_obj.get("name"))
            action.triggered.connect(
                partial(self.slot_action_clicked, sub_obj))
