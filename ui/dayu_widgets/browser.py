    ...
    def slot_save_file(self):
        ...
        file_name, selected_filter = QtWidgets.QFileDialog.getSaveFileName(
            self, "Save File", os.path.join(
                initial_dir, default_name), filter_string, self._last_used_filter)
        ...
        if file_name:
            ...
            selected_type = next(
                (ft for ft in self._file_types if ft[0] in selected_filter), None)
            if selected_type:
                valid_extensions = selected_type[1] if isinstance(
                    selected_type[1], list) else [selected_type[1]]
...
class MDragFolderButton(MToolButton):
    ...
    def dragEnterEvent(self, event):
        ...
        if event.mimeData().hasFormat("text/uri-list"):
            folder_list = [
                url.toLocalFile() for url in event.mimeData().urls() if os.path.isdir(
                    url.toLocalFile())]
