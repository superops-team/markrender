        ...
        if role in SETTING_MAP.keys():
            ...
            if not formatter_from_config and role not in [
                ...
                value = apply_formatter(
                    formatter_from_config, get_obj_value(
                        data_obj, attr), data_obj)
            formatter_from_model = SETTING_MAP[role].get(
                "formatter", None)  # role 配置的转换函数
            result = apply_formatter(formatter_from_model, value)
            return result

        if role == QtCore.Qt.CheckStateRole and attr_dict.get(
                "checkable", False):
                ...
                if parent_index.isValid():
                    ...
                    if new_parent_value != old_parent_value:
                        ...
                        self.dataChanged.emit(
                            parent_index, parent_index, [role])
            else:
                set_obj_value(data_obj, key, value)
                self.dataChanged.emit(index, index, [role])
            return True
...
class MSortFilterModel(QtCore.QSortFilterProxyModel):
    def __init__(self, parent=None):
        ...
        self.search_reg.setPatternOptions(
            QtCore.QRegularExpression.CaseInsensitiveOption)
        # This sets a wildcard-like pattern in regex
