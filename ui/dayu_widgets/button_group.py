...
class MCheckBoxGroup(MButtonGroupBase):
    ...
    def _slot_map_signal(self, button=None):
        self.sig_checked_changed.emit([check_box.text(
        ) for check_box in self._button_group.buttons() if check_box.isChecked()])
    ...
    def get_dayu_checked(self):
        return [check_box.text() for check_box in self._button_group.buttons()
...
class MRadioButtonGroup(MButtonGroupBase):
    ...
    def _on_button_clicked(self, button):
        self.sig_checked_changed.emit(
            self._button_group.id(button), button.text())
