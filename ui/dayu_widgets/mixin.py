...
def focus_shadow_mixin(cls):
    ...
    def _new_focus_in_event(self, *args, **kwargs):
        ...
        if not self.graphicsEffect():
            ...
            color = vars(dayu_theme).get(
                "{}_color".format(
                    dayu_type or "primary"))
...
def stacked_animation_mixin(cls):
    ...
    """
    if not _stackable(
            cls):  # If widget can't stack, return the original widget class
        return cls
    ...
    def _play_anim(self, index):
        ...
        if self._previous_index < index:
            ...
            self._to_hide_pos_ani.setStartValue(
