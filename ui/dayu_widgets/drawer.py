...
class MDrawer(QtWidgets.QWidget):
    ...
    def _set_proper_position(self):
        ...
        if self._position == MDrawer.LeftPos:
            pos = parent_geo.topLeft() if parent.parent(
            ) is None else parent.mapToGlobal(parent_geo.topLeft())
            target_x = pos.x()
            target_y = pos.y()
        ...
        if self._position == MDrawer.RightPos:
            pos = parent_geo.topRight() if parent.parent(
            ) is None else parent.mapToGlobal(parent_geo.topRight())
            self.setFixedHeight(parent_geo.height())
        ...
        if self._position == MDrawer.BottomPos:
            pos = parent_geo.bottomLeft() if parent.parent(
            ) is None else parent.mapToGlobal(parent_geo.bottomLeft())
            self.setFixedWidth(parent_geo.width())
            target_x = pos.x()
