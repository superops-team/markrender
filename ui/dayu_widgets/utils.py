...
def generate_color(primary_color, index):
    ...
    light = index <= 6
    hsv_color = QtGui.QColor(primary_color) if isinstance(
        primary_color, six.string_types) else primary_color
...
def _(input_object):
    attr_list = ["icon"]
    path = next((get_obj_value(input_object, attr)
                for attr in attr_list), None)
    return icon_formatter(path)
...
def overflow_format(num, overflow):
    ...
    if not isinstance(num, int):
        raise ValueError(
            "Input argument 'num' should be int type, "
            "but get {}".format(
                type(num)))
...
def read_settings(organization, app_name):
    ...
    for grp_name in settings.childGroups():
        ...
        result_dict.update({grp_name + "/" + key: settings.value(key)
...
def add_settings(organization, app_name, event_name="closeEvent"):
    ...
    def bind(self, attr, widget, property, default=None, formatter=None):
        ...
        if property == "geometry":  # 窗口大小位置需要特殊处理
            if isinstance(
                    value, QtCore.QRect):  # setting 并没有存，使用用户default传入进来的geo
...
def get_fit_geometry():
    geo = next((screen.availableGeometry()
                for screen in QtWidgets.QApplication.screens()), None, )
    return QtCore.QRect(
        geo.width() / 4,
        geo.height() / 4,
        geo.width() / 2,
