...
class MPage(QtWidgets.QWidget, MFieldMixin):
    ...
    def __init__(self, parent=None):
        ...
        self.register_field("total_page", lambda: utils.get_total_page(
            self.field("total"), self.field("page_size_selected")), )
        self.register_field(
            "total_page_text", lambda: str(
                self.field("total_page")))
        self.register_field(
            "display_text",
        ...
        self.register_field("can_next", lambda: self.field(
            "current_page") < self.field("total_page"))
        ...
        self._pre_button.clicked.connect(
            functools.partial(
                self._slot_change_current_page, -1))
    ...
    def set_page_config(self, data_list):
        ...
        self.set_field("page_size_list", [{"label": str(data), "value": data} if isinstance(
            data, int) else data for data in data_list], )
