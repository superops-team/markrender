...
class ScrollableMenuBase(QtWidgets.QMenu):
    ...
    def setMaxItemCount(self, count):
        ...
        self.defaultItemHeight = style.sizeFromContents(
            QtWidgets.QStyle.CT_MenuItem, opt, size, self).height()

        if not count:
            self.setMaximumHeight(self._maximumHeight)
        else:
            fw = style.pixelMetric(
                QtWidgets.QStyle.PM_MenuPanelWidth, None, self)
            vmargin = style.pixelMetric(
                QtWidgets.QStyle.PM_MenuHMargin, opt, self)
    ...
    def scrollHeight(self, style):
        return style.pixelMetric(
            QtWidgets.QStyle.PM_MenuScrollerHeight, None, self) * 2

    def isScrollable(self):
        return self.property("scrollable") and self.height() < super(
            ScrollableMenuBase, self).sizeHint().height()
    ...
    def checkScroll(self):
        ...
        if self.scrollBy(delta):
            self.scrollTimer.start(
                self.scrollTimer.property("defaultInterval"))
    ...
    def ensureVisible(self, action):
        ...
        hmargin = style.pixelMetric(
            QtWidgets.QStyle.PM_MenuHMargin, None, self)
        vmargin = style.pixelMetric(
            QtWidgets.QStyle.PM_MenuVMargin, None, self)
        scrollHeight = self.scrollHeight(style)
        ...
        if step < 0:
            ...
            for lastAction in reversed(self.actions()):
                if lastAction.isVisible():
                    break
            lastBottom = self.actionGeometry(
                lastAction).bottom() - newDelta + scrollHeight
    ...
    def event(self, event):
        ...
        if event.type() == event.KeyPress and event.key() in (
            ...
            pos = event.pos()
            if self.scrollUpRect.contains(
                    pos) or self.scrollDownRect.contains(pos):
            ...
            if not (self.scrollUpRect.contains(pos)
                    or self.scrollDownRect.contains(pos)):
    ...
    def paintEvent(self, event):
        ...
        # offset translation is required in order to allow correct fade
        # animations
        ...
        for isDown, enabled, scrollRect in upData, downData:
            ...
            if enabled:
                if scrollRect.contains(cursor):
                    ...
                    style.drawControl(
                        QtWidgets.QStyle.CE_MenuItem, frame, qp, self)
            ...
            style.drawControl(
                QtWidgets.QStyle.CE_MenuScroller,
                scrollOpt,
                qp,
                self)
        ...
        if fw:
            ...
            frame.lineWidth = style.pixelMetric(
                QtWidgets.QStyle.PM_MenuPanelWidth)
...
class MMenu(SearchableMenuBase):
    ...
    def set_value(self, data):
        assert isinstance(
            data, (list, six.string_types, six.integer_types, float))
    ...
    def _add_menu(self, parent_menu, data_dict, long_path=None):
        if "children" in data_dict:
            ...
            for i in data_dict.get("children"):
                ...
                assemble_long_path = "{root}/{label}".format(
                    root=long_path, label=i.get("label"))
    ...
    def set_data(self, option_list):
        ...
        if option_list:
            ...
            if all(isinstance(i, (int, float)) for i in option_list):
                option_list = [
                    {"value": i, "label": str(i)} for i in option_list]
        # 全部转换成 dict 类型的 list
    ...
    def slot_on_action_triggered(self, action):
        ...
        if self.property("cascader"):
            ...
            if self._action_group.isExclusive():
                ...
                selected_data = [
                    act.property("value") for act in self._action_group.actions() if act.isChecked()]
